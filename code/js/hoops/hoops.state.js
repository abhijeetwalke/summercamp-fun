/* ============================================================
   Basketball — "Hardwood" — State manager  [SUBJECT: hoops]
   localStorage persistence, per-device (no backend, no auth).
   Own key so it never collides with Math or World Awareness.
   Tracks: lessons done + best score, subsection badges (Trophy
   Case), day-streak, personal-best, debate cards seen.
   ============================================================ */

window.HOOPS = window.HOOPS || {};

HOOPS.State = (function () {
  var KEY = "summer-camp-hoops-v1";
  var cache = null;
  ...
  function fresh() {
    return {
      ...
      profile: { nickname: "", theme: "dark" },  // Hardwood is dark by default
      lessons: {},                    // id -> { done, best, attempts, lastScore, completedAt }
      sections: {},                   // subsectionId -> { quizBest, badge:true }
      personalBest: 0,                // best single quiz score across all of Basketball
  ...
  /* ---------- subsection review quizzes + badges (Trophy Case) ---------- */
  function recordSectionQuiz(secId, scorePct, passMark) {
    var s = load(), t = Date.now();
    var rec = s.sections[secId] || { quizBest: 0, badge: false };
    if (scorePct > rec.quizBest) rec.quizBest = scorePct;
    var earnedNow = false;
    if (scorePct >= passMark && !rec.badge) { rec.badge = true; earnedNow = true; }
    s.sections[secId] = rec;
    s.log.unshift({ t: t, kind: "review", id: secId, score: scorePct });
    s.log = s.log.slice(0, 60);
    var isPB = recordScore(scorePct);
    touchStreak(); save();
    return { earnedNow: earnedNow, isPB: isPB };
  }
  function hasBadge(secId) { var s = load(); return !!(s.sections[secId] && s.sections[secId].badge); }
  function sectionQuizBest(secId) { var s = load(); return (s.sections[secId] || {}).quizBest || 0; }
  ...
  function sectionProgress(sec) {
    var s = load(), total = sec.lessons.length, done = 0;
    sec.lessons.forEach(function (lid) { if (s.lessons[lid] && s.lessons[lid].done) done += 1; });
    return { done: done, total: total, pct: total ? Math.round(done / total * 100) : 0,
             allDone: done === total && total > 0 };
  }
  function totalsAcross(sections) {
    var lessonsDone = 0, lessonsTotal = 0, badges = 0;
    sections.forEach(function (sec) {
      var p = sectionProgress(sec);
      lessonsDone += p.done; lessonsTotal += p.total;
      if (hasBadge(sec.id)) badges += 1;
    });
  ...
  return {
    ...
    recordSectionQuiz: recordSectionQuiz, hasBadge: hasBadge, sectionQuizBest: sectionQuizBest,
    seeDebate: seeDebate, debatesSeenCount: debatesSeenCount,
    sectionProgress: sectionProgress, totalsAcross: totalsAcross, bestScoreLesson: bestScoreLesson,
    sectionUnlocked: sectionUnlocked, lessonUnlocked: lessonUnlocked, reviewUnlocked: reviewUnlocked,
